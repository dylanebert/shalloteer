...
async function init() {
  ...
  if (!projectName) {
    ...
    console.log('  ' + cyan('npm create shalloteer@latest <project-name>'));
    process.exit(1);
  }
  ...
  console.log();
  console.log(`${cyan('Creating Shalloteer project in')} ${green(projectName)}`);
  console.log();
  ...
  // Check if bun is available even if not used to run create-shalloteer
  ...
  try {
    ...
    // Copy llms.txt from installed shalloteer package
    const targetLlmsPath = path.join(targetDir, 'llms.txt');
    const shalloteerLlmsPath = path.join(targetDir, 'node_modules', 'shalloteer', 'llms.txt');
    
    if (fs.existsSync(shalloteerLlmsPath)) {
      fs.copyFileSync(shalloteerLlmsPath, targetLlmsPath);
