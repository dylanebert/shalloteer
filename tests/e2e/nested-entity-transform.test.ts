...
import { State, TIME_CONSTANTS, XMLParser } from 'shalloteer';
...
describe('E2E: Nested Entity Transform Hierarchy', () => {
  ...
  it('should establish parent-child relationship for nested entities', () => {
    ...
    const entities = state.query(Transform);
  ...
  it('should handle multi-level nested entities', () => {
    const initialEntityCount = state.query(Transform).length;
    ...
    const allEntities = state.query(Transform);
