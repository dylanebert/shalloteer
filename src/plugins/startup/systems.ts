...
import { NULL_ENTITY, type System } from '../../core';
...
export const LightingStartupSystem: System = {
  group: 'setup',
  update: (state) => {
    const existingHemisphereLight = state.query(Ambient);
    const existingDirectionalLight = state.query(Directional);
...
export const PlayerStartupSystem: System = {
  group: 'setup',
  update: (state) => {
    const existingPlayers = state.query(Player);
...
export const CameraStartupSystem: System = {
  group: 'setup',
  update: (state) => {
    const existingCameras = state.query(MainCamera);
...
export const PlayerCharacterSystem: System = {
  group: 'setup',
  update(state) {
    const playersWithoutCharacter = state
      .query(Player)
