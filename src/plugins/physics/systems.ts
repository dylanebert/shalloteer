...
import { TIME_CONSTANTS } from '../../core';
...
export const PhysicsWorldSystem: System = {
  ...
  update: (state) => {
    ...
    const worldEntities = state.query(PhysicsWorld);
...
export const ApplyForcesSystem: System = {
  ...
  update: (state) => {
    const context = getPhysicsContext(state);
    for (const entity of state.query(ApplyForce, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const ApplyTorquesSystem: System = {
  ...
  update: (state) => {
    const context = getPhysicsContext(state);
    for (const entity of state.query(ApplyTorque, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const ApplyImpulsesSystem: System = {
  ...
  update: (state) => {
    const context = getPhysicsContext(state);
    for (const entity of state.query(ApplyImpulse, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const ApplyAngularImpulsesSystem: System = {
  ...
  update: (state) => {
    const context = getPhysicsContext(state);
    for (const entity of state.query(ApplyAngularImpulse, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const SetVelocitySystem: System = {
  ...
  update: (state) => {
    ...
    for (const entity of state.query(SetLinearVelocity, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
    ...
    for (const entity of state.query(SetAngularVelocity, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const KinematicMovementSystem: System = {
  ...
  update: (state) => {
    ...
    for (const entity of state.query(KinematicMove, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
    ...
    for (const entity of state.query(KinematicRotate, Body)) {
      const body = context.entityToRigidbody.get(entity);
      if (body) {
...
export const CollisionEventCleanupSystem: System = {
  group: 'setup',
  update: (state) => {
    for (const entity of state.query(TouchedEvent)) {
      state.removeComponent(entity, TouchedEvent);
    }

    for (const entity of state.query(TouchEndedEvent)) {
...
export const PhysicsRapierSyncSystem: System = {
  ...
  update: (state) => {
    ...
    for (const [entity, body] of context.entityToRigidbody) {
      if (state.hasComponent(entity, Body)) {
        syncRigidbodyToECS(entity, body);
