...
export const PlayerMovementSystem: System = {
  group: 'fixed',
  update: (state) => {
    const playerEntities = state.query(
      Player,
      CharacterMovement,
      Transform,
      Body,
    ...
    const cameraEntities = state.query(OrbitCamera);
...
export const PlayerCameraLinkingSystem: System = {
  group: 'simulation',
  update: (state) => {
    const players = state.query(Player);
    const cameras = state.query(OrbitCamera);
...
export const PlayerCameraControlSystem: System = {
  ...
  update: (state) => {
    const playerEntities = state.query(Player, InputState);
